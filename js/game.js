import { board, mineCounter, newGameBtn } from './elements.js'
...
export function startGame(mineCount) {
  board.onclick = e => {
    if (e.target.matches('td')) {
      ...
      board.onclick = e => {
        if (e.target.matches('td:not(.flag)')) {
          open(e.target)
        }
        if (e.target.matches('[data-mine]') === true) {
          board.onclick = null
          board.oncontextmenu = null
          stopSecondCounter();
          newGameBtn.style.backgroundImage = 'url("images/sad.png")';
        }
      }
